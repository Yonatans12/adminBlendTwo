...
const Language = () => {
  ...
  let Rowdata =[...( LanguageData?.Language?.r ?? [])]?.reverse()?.map((data, i) => ({
    rowid: i + 1 + page * 10,
    ...data,
  }));
  ...
  useEffect(() => {
    dispatch(Language_Listing(page * 10));
  }, []);
  ...
  let handeldPromptAction = () => {
    ...
    dispatch(
      typeCheck === 2 ? HandeldLanguage(formdata) : AddLanguage(formdata)
    )
      .unwrap()
      .then((result) => {
        if (result?.s === 1) {
    ...
    try {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          dispatch(HandeldLanguage(formdata))
            .unwrap()
            .then((result) => {
              if (result?.s === 1) {
                dispatch(Language_Listing(page * 10));
                clearEditField();
                toast("Langauage deleted !", {
                  icon: "👏",
                  style: {
                    borderRadius: "10px",
                    background: "#333",
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {typeCheck === 1 ? " Enter Language" : " Edit Language"}
            </InputLabel>
            {typeCheck === 2 ? (
              <Typography
